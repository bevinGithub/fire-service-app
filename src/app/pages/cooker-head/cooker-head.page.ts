...
@Component({
  selector: 'app-cooker-head',
  templateUrl: './cooker-head.page.html',
  styleUrls: ['./cooker-head.page.scss'],
})
export class CookerHeadPage implements OnInit {
  stairPressure: any = {};
  certID: any;
  cert: any;
  networkStatus: any;
  ...
  submitstairPressures() {
    ...
    if (this.networkStatus === 'none') {
      const bsSql = 'SELECT * FROM fire_cooker_head_suppression WHERE service_cert_id=?';
      this.database.executeSql(bsSql, [this.certID]).then((bsRes: any) => {
        ...
        if (bsRes.rows.length > 0) {
          ...
          const isSync = 'No';
          // eslint-disable-next-line max-len
          this.database.executeSql(`INSERT INTO fire_cooker_head_suppression (service_type_id, site_id, service_cert_id, tech_id, location_1, functional_1, comment_1, location_2, functional_2, comment_2, location_3, functional_3, comment_3, location_4, functional_4, comment_4, location_5, functional_5, comment_5, isSync, created_on) VALUES ('${this.stairPressure.service_type_id}','${this.stairPressure.site_id}','${this.stairPressure.service_cert_id}','${this.stairPressure.tech_id}','${this.location1}','${this.functional1}','${this.comment1}','${this.location2}', '${this.functional2}', '${this.comment2}', '${this.location3}', '${this.functional3}', '${this.comment3}', '${this.location4}', '${this.functional4}', '${this.comment4}', '${this.location5}', '${this.functional5}', '${this.comment5}', '${isSync}','${this.stairPressure.created_on}')`, [])
          .then((bs: any) => {
            console.log('fire_cooker_head_suppression ADDED: ' + JSON.stringify(bs));
            this.presentToast('Cooker head suppression successfully saved!');
            this.modalController.dismiss();
          }, err => {
  ...
  dropTable() {
    this.database.executeSql(`DROP table fire_cooker_head_suppression `, [])
      .then((resAdd: any) => {
        console.log('TABLE fire_cooker_head_suppression REMOVED: ' + JSON.stringify(resAdd));
      });
  }
}
