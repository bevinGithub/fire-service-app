...
@Component({
  selector: 'app-wire-less',
  templateUrl: './wire-less.page.html',
  styleUrls: ['./wire-less.page.scss'],
})
export class WireLessPage implements OnInit {
  sKnock: any = {};
  public devices: any;
  certID: any;
  cert: any;
  ...
  ngOnInit() {
    ...
    console.log(this.siteID + '------' + this.serviceTypeID);
    if (this.networkStatus === 'none') { //Offline
      ...
      this.sqlite.create({
        ...
        this.database.executeSql(`SELECT * FROM fire_template_device_loops_table_wireless WHERE service_type_id=? AND site_id=?`, [this.serviceTypeID, this.siteID])
        .then((res: any) => {
  ...
  onSubmit() {
    ...
    for(let i=0; i < this.fieldsCounter.length; i++) {
      ...
      // eslint-disable-next-line max-len
      this.database.executeSql(`UPDATE fire_template_device_loops_table_wireless SET na=?, cleaned=?, tested=?, correct_message=?, new_message=?, isSync=? WHERE id=${id}`, updateValues)
      .then((res: any) => {
        console.log(res);
      }, err => {
        console.log('fire_template_device_loops_table_wireless ERROR: ' + JSON.stringify(err));
      });
    }
    this.presentToast('Wireless device testing successfully saved!');
