...
import { TagsActuatorDataPage } from 'src/app/modals/tags-actuator-data/tags-actuator-data.page';

@Component({
  selector: 'app-smoke-read-write-actuator',
  templateUrl: './smoke-read-write-actuator.page.html',
  styleUrls: ['./smoke-read-write-actuator.page.scss'],
})
export class SmokeReadWriteActuatorPage implements OnInit {
  url = environment.url;
  techID: any;
  deviceData: any;
  config: any;
  ...
  public siteID: any;
  public certID: any;
  public actuatorNumber: any;
  public location: any;
  public tagDate: any;
  public serviceID: any;
  public serviceTypeID: any;
  public actuatorCode: any;
  public cpID: any;
  ...
  ionViewWillEnter(){
    ...
    if (this.state === 'read') {
      this.pageTitle = 'READING RFID: ACTUATOR';
      this.readingTag = true;
      this.nfcReadData();
    }
    if (this.state === 'write') {
      this.pageTitle = 'WRITING TO RFID: ACTUATOR';
  ...
  async getTagData(tag) {
    ...
    if (tag.ndefMessage[2]) {
      this.actuatorCode  = this.nfc.bytesToString(tag.ndefMessage[2].payload).substring(3);
    } else {
      this.actuatorCode = '';
    }
    if (tag.ndefMessage[1]) {
      this.actuatorNumber  = this.nfc.bytesToString(tag.ndefMessage[1].payload).substring(3);
    } else {
      this.actuatorNumber = '';
    }
    ...
    console.log('SiteID: ' + this.siteID + ' ---Fan Code: ' + this.actuatorCode + ' ---Fan Number: ' + this.actuatorNumber  + '----Location: ' + this.location + '----: Date Scanned: '
    ...
    if (this.siteID !== '' && this.serviceID === '' && this.serviceTypeID === '') {
      ...
      const readData = {
        ...
        actuatorNumber: this.actuatorNumber,
        location: this.location,
        techID: this.techID,
        cpID: this.cpID,
      };
      console.log(readData);
      this.http.post(this.url + 'sp-post-tag-actuator-data.php', readData).subscribe((data: any) => {
        console.log(data);
...
writeTag() {
  ...
  this.platform.ready().then(() => {
    ...
    this.listener = this.nfc.addNdefListener((res: any) => {
      ...
      const actuatorNumber = this.ndef.textRecord(this.tag.actuatorNumber);
      const actuatorCode = this.ndef.textRecord(this.tag.actuatorCode);
      ...
      this.nfc.write([siteID,actuatorNumber, actuatorCode,location,date,serviceID,serviceTypeID,cpID]).then(success => {
