...
@Component({
  selector: 'app-test-double-knock',
  templateUrl: './test-double-knock.page.html',
  styleUrls: ['./test-double-knock.page.scss'],
})
export class TestDoubleKnockPage implements OnInit {
  sKnock: any = {};
  public devices: any;
  certID: any;
  siteID: any;
  ...
  ngOnInit() {
    ...
    if (this.networkStatus === 'none') { //Offline
      ...
      this.sqlite.create({
        ...
        this.database.executeSql(`SELECT * FROM fire_template_device_loops_table_sounder_knock WHERE service_type_id=? AND site_id=? AND panel_id=?`, [this.serviceTypeID, this.siteID, this.panelID])
        .then((res: any) => {
  ...
  onSubmit() {
    ...
    for(let i=0; i < this.fieldsCounter.length; i++) {
      ...
      // eslint-disable-next-line max-len
      this.database.executeSql(`UPDATE fire_template_device_loops_table_sounder_knock SET na=?, cleaned=?, tested=?, correct_message=?, new_message=?, isSync=? WHERE id=${id}`, updateValues)
      .then((res: any) => {
        console.log(res);
      }, err => {
    ...
    this.presentToast('Testing devices double Knock successfully saved!');
