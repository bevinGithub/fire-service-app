...
@Component({
  selector: 'app-smoke-read-write-fan',
  templateUrl: './smoke-read-write-fan.page.html',
  styleUrls: ['./smoke-read-write-fan.page.scss'],
})
export class SmokeReadWriteFanPage implements OnInit {
  url = environment.url;
  techID: any;
  deviceData: any;
  config: any;
  ...
  ionViewWillEnter(){
    ...
    if (this.state === 'read') {
      this.pageTitle = 'READING RFID: FAN';
      this.readingTag = true;
      this.nfcReadData();
    }
    if (this.state === 'write') {
      this.pageTitle = 'WRITING TO RFID: FAN';
  ...
  async getTagData(tag) {
    ...
    if (tag.ndefMessage[2]) {
      this.fanCode  = this.nfc.bytesToString(tag.ndefMessage[2].payload).substring(3);
    } else {
      this.fanCode = '';
    }
    if (tag.ndefMessage[1]) {
      this.fanNumber  = this.nfc.bytesToString(tag.ndefMessage[1].payload).substring(3);
    } else {
      this.fanNumber = '';
    }
    ...
    console.log('SiteID: ' + this.siteID + ' ---Fan Code: ' + this.fanCode + ' ---Fan Number: ' + this.fanNumber  + '----Location: ' + this.location + '----: Date Scanned: '
    ...
    if (this.siteID !== '' && this.serviceID === '' && this.serviceTypeID === '') {
      ...
      this.http.post(this.url + 'sp-post-tag-fan-data.php', readData).subscribe((data: any) => {
        console.log(data);
...
writeTag() {
  ...
  this.platform.ready().then(() => {
    ...
    this.listener = this.nfc.addNdefListener((res: any) => {
      ...
      const fanNumber = this.ndef.textRecord(this.tag.fanNumber);
      const fanCode = this.ndef.textRecord(this.tag.fanCode);
      ...
      this.nfc.write([siteID,fanNumber, fanCode,location,date,serviceID,serviceTypeID,cpID]).then(success => {
