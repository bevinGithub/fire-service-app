...
@Component({
  selector: 'app-smoke-extraction',
  templateUrl: './smoke-extraction.page.html',
  styleUrls: ['./smoke-extraction.page.scss'],
})
export class SmokeExtractionPage implements OnInit {
  stairPressure: any = {};
  certID: any;
  cert: any;
  networkStatus: any;
  ...
  constructor(
    ...
    this.sqlite.create({
      ...
      this.database = db;
      //  this.dropTable();
      this.database.executeSql(`CREATE TABLE IF NOT EXISTS fire_smoke_extraction_fans  (
        ...
        created_on TEXT)`, []).then((res: any) => {
          console.log('fire_smoke_extraction_fans table Created: ' + JSON.stringify(res));
        });
        ...
        if (this.networkStatus === 'none') { //Offline
          const stairPressureSql = 'SELECT * FROM fire_smoke_extraction_fans WHERE service_cert_id=?';
  ...
  submitstairPressures() {
    ...
    if (this.networkStatus === 'none') {
      const bsSql = 'SELECT * FROM fire_smoke_extraction_fans WHERE service_cert_id=?';
  ...
  dropTable() {
    this.database.executeSql(`DROP table fire_smoke_extraction_fans `, [])
      .then((resAdd: any) => {
        console.log('TABLE fire_smoke_extraction_fans REMOVED: ' + JSON.stringify(resAdd));
      });
  }
}
