...
@Component({
  selector: 'app-suppression-cylinder',
  templateUrl: './suppression-cylinder.page.html',
  styleUrls: ['./suppression-cylinder.page.scss'],
})
export class SuppressionCylinderPage implements OnInit {
  stairPressure: any = {};
  certID: any;
  cert: any;
  networkStatus: any;
  ...
  constructor(
    ...
    this.sqlite.create({
      ...
      this.database = db;
      //  this.dropTable();
      this.database.executeSql(`CREATE TABLE IF NOT EXISTS fire_suppression_cylinder  (
        ...
        created_on TEXT)`, []).then((res: any) => {
          console.log('fire_suppression_cylinder table Created: ' + JSON.stringify(res));
        });
        ...
        if (this.networkStatus === 'none') { //Offline
          const stairPressureSql = 'SELECT * FROM fire_suppression_cylinder WHERE service_cert_id=?';
  ...
  submitstairPressures() {
    ...
    if (this.networkStatus === 'none') {
      const bsSql = 'SELECT * FROM fire_suppression_cylinder WHERE service_cert_id=?';
      this.database.executeSql(bsSql, [this.certID]).then((bsRes: any) => {
        ...
        if (bsRes.rows.length > 0) {
          ...
          // eslint-disable-next-line max-len
          this.database.executeSql(`UPDATE fire_suppression_cylinder SET service_type_id=?, site_id=?, service_cert_id=?, tech_id=?, location_1=?, functional_1=?, comment_1=?, location_2=?, functional_2=?, comment_2=?, location_3=?, functional_3=?, comment_3=?, location_4=?, functional_4=?, comment_4=?, location_5=?, functional_5=?, comment_5=? WHERE id=${pressure.id}`, updateData)
          .then((bs: any) => {
            console.log('fire_suppression_cylinder UPDATED: ' + JSON.stringify(bs));
            this.presentToast('Suppression cylinders test successfully saved!');
            this.modalController.dismiss();
          }, err => {
            console.log('fire_suppression_cylinder ERROR: ' + JSON.stringify(err));
            this.presentToast('Suppression cylinders test could not be saved!');
          });
  ...
  dropTable() {
    this.database.executeSql(`DROP table fire_suppression_cylinder `, [])
      .then((resAdd: any) => {
        console.log('TABLE fire_suppression_cylinder REMOVED: ' + JSON.stringify(resAdd));
      });
  }
}
