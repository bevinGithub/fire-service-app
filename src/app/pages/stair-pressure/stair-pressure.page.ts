...
@Component({
  selector: 'app-stair-pressure',
  templateUrl: './stair-pressure.page.html',
  styleUrls: ['./stair-pressure.page.scss'],
})
export class StairPressurePage implements OnInit {
  stairPressure: any = {};
  certID: any;
  cert: any;
  networkStatus: any;
  ...
  constructor(
    ...
    this.sqlite.create({
      ...
      this.database = db;
      //  this.dropTable();
      this.database.executeSql(`CREATE TABLE IF NOT EXISTS fire_stair_pressure_fans  (
        ...
        created_on TEXT)`, []).then((res: any) => {
          console.log('fire_stair_pressure_fans table Created: ' + JSON.stringify(res));
        });
        ...
        console.log('Service ID: ' + this.certID);
        if (this.networkStatus === 'none') { //Offline
          const stairPressureSql = 'SELECT * FROM fire_stair_pressure_fans WHERE service_cert_id=?';
  ...
  submitstairPressures() {
    ...
    if (this.networkStatus === 'none') {
      const bsSql = 'SELECT * FROM fire_stair_pressure_fans WHERE service_cert_id=?';
      this.database.executeSql(bsSql, [this.certID]).then((bsRes: any) => {
        ...
        if (bsRes.rows.length > 0) {
          ...
          // eslint-disable-next-line max-len
          this.database.executeSql(`UPDATE fire_stair_pressure_fans SET service_type_id=?, site_id=?, service_cert_id=?, tech_id=?, location_1=?, functional_1=?, comment_1=?, location_2=?, functional_2=?, comment_2=?, location_3=?, functional_3=?, comment_3=?, location_4=?, functional_4=?, comment_4=?, location_5=?, functional_5=?, comment_5=? WHERE id=${pressure.id}`, updateData)
          .then((bs: any) => {
            console.log('fire_stair_pressure_fans UPDATED: ' + JSON.stringify(bs));
            this.presentToast('Stair pressure successfully saved!');
            this.modalController.dismiss();
          }, err => {
            console.log('fire_stair_pressure_fans ERROR: ' + JSON.stringify(err));
            this.presentToast('Stair pressure could not be saved!');
          });
  ...
  dropTable() {
    this.database.executeSql(`DROP table fire_stair_pressure_fans `, [])
      .then((resAdd: any) => {
        console.log('TABLE fire_stair_pressure_fans REMOVED: ' + JSON.stringify(resAdd));
      });
  }
