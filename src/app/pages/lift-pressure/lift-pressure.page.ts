...
@Component({
  selector: 'app-lift-pressure',
  templateUrl: './lift-pressure.page.html',
  styleUrls: ['./lift-pressure.page.scss'],
})
export class LiftPressurePage implements OnInit {
  stairPressure: any = {};
  certID: any;
  cert: any;
  networkStatus: any;
  ...
  constructor(
    ...
    this.sqlite.create({
      ...
      this.database = db;
      //  this.dropTable();
      this.database.executeSql(`CREATE TABLE IF NOT EXISTS fire_lift_pressure_fans  (
        ...
        created_on TEXT)`, []).then((res: any) => {
          console.log('fire_lift_pressure_fans table Created: ' + JSON.stringify(res));
        });
    });
  }
  ...
  ionViewWillEnter(){
    ...
    if (this.networkStatus === 'none') { //Offline
      const stairPressureSql = 'SELECT * FROM fire_lift_pressure_fans WHERE service_cert_id=?';
  ...
  submitstairPressures() {
    ...
    if (this.networkStatus === 'none') {
      const bsSql = 'SELECT * FROM fire_lift_pressure_fans WHERE service_cert_id=?';
      this.database.executeSql(bsSql, [this.certID]).then((bsRes: any) => {
        ...
        if (bsRes.rows.length > 0) {
          ...
          const isSync = 'No';
          // eslint-disable-next-line max-len
          this.database.executeSql(`INSERT INTO fire_lift_pressure_fans (service_type_id, site_id, service_cert_id, tech_id, location_1, functional_1, comment_1, location_2, functional_2, comment_2, location_3, functional_3, comment_3, location_4, functional_4, comment_4, location_5, functional_5, comment_5, isSync, created_on) VALUES ('${this.stairPressure.service_type_id}','${this.stairPressure.site_id}','${this.stairPressure.service_cert_id}','${this.stairPressure.tech_id}','${this.location1}','${this.functional1}','${this.comment1}','${this.location2}', '${this.functional2}', '${this.comment2}', '${this.location3}', '${this.functional3}', '${this.comment3}', '${this.location4}', '${this.functional4}', '${this.comment4}', '${this.location5}', '${this.functional5}', '${this.comment5}', '${isSync}','${this.stairPressure.created_on}')`, [])
          .then((bs: any) => {
            console.log('fire_lift_pressure_fans ADDED: ' + JSON.stringify(bs));
            this.presentToast('Stair pressure successfully saved!');
            this.modalController.dismiss();
          }, err => {
            console.log('fire_lift_pressure_fans ERROR: ' + JSON.stringify(err));
  ...
  dropTable() {
    this.database.executeSql(`DROP table fire_lift_pressure_fans `, [])
      .then((resAdd: any) => {
