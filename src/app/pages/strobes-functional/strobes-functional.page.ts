...
@Component({
  selector: 'app-strobes-functional',
  templateUrl: './strobes-functional.page.html',
  styleUrls: ['./strobes-functional.page.scss'],
})
export class StrobesFunctionalPage implements OnInit {
  stairPressure: any = {};
  certID: any;
  cert: any;
  networkStatus: any;
  ...
  constructor(
    ...
    this.sqlite.create({
      ...
      this.database = db;
      //  this.dropTable();
      this.database.executeSql(`CREATE TABLE IF NOT EXISTS fire_strobes_functional  (
        ...
        created_on TEXT)`, []).then((res: any) => {
          console.log('fire_strobes_functional table Created: ' + JSON.stringify(res));
        });
        ...
        if (this.networkStatus === 'none') { //Offline
          const stairPressureSql = 'SELECT * FROM fire_strobes_functional WHERE service_cert_id=?';
  ...
  submitstairPressures() {
    ...
    if (this.networkStatus === 'none') {
      const bsSql = 'SELECT * FROM fire_strobes_functional WHERE service_cert_id=?';
      this.database.executeSql(bsSql, [this.certID]).then((bsRes: any) => {
        ...
        if (bsRes.rows.length > 0) {
          ...
          // eslint-disable-next-line max-len
          this.database.executeSql(`UPDATE fire_strobes_functional SET service_type_id=?, site_id=?, service_cert_id=?, tech_id=?, location_1=?, functional_1=?, comment_1=?, location_2=?, functional_2=?, comment_2=?, location_3=?, functional_3=?, comment_3=?, location_4=?, functional_4=?, comment_4=?, location_5=?, functional_5=?, comment_5=? WHERE id=${pressure.id}`, updateData)
          .then((bs: any) => {
            console.log('fire_strobes_functional UPDATED: ' + JSON.stringify(bs));
            this.presentToast('Strobes functional successfully saved!');
            this.modalController.dismiss();
          }, err => {
            console.log('fire_strobes_functional ERROR: ' + JSON.stringify(err));
            this.presentToast('Strobes functional could not be saved!');
          });
  ...
  dropTable() {
    this.database.executeSql(`DROP table fire_strobes_functional `, [])
      .then((resAdd: any) => {
        console.log('TABLE fire_strobes_functional REMOVED: ' + JSON.stringify(resAdd));
      });
  }
}
