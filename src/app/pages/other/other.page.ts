...
export class OtherPage implements OnInit {
  ...
  constructor(
    ...
    this.sqlite.create({
      ...
      this.database = db;
      //  this.dropTable();
      this.database.executeSql(`CREATE TABLE IF NOT EXISTS fire_other_15  (
        ...
        created_on TEXT)`, []).then((res: any) => {
          console.log('fire_other_15 table Created: ' + JSON.stringify(res));
        });
        ...
        if (this.networkStatus === 'none') { //Offline
          const stairPressureSql = 'SELECT * FROM fire_other_15 WHERE service_cert_id=?';
  ...
  submitstairPressures() {
    ...
    if (this.networkStatus === 'none') {
      const bsSql = 'SELECT * FROM fire_other_15 WHERE service_cert_id=?';
      this.database.executeSql(bsSql, [this.certID]).then((bsRes: any) => {
        ...
        if (bsRes.rows.length > 0) {
          ...
          // eslint-disable-next-line max-len
          this.database.executeSql(`UPDATE fire_other_15 SET service_type_id=?, site_id=?, service_cert_id=?, tech_id=?, location_1=?, functional_1=?, comment_1=?, location_2=?, functional_2=?, comment_2=?, location_3=?, functional_3=?, comment_3=?, location_4=?, functional_4=?, comment_4=?, location_5=?, functional_5=?, comment_5=? WHERE id=${pressure.id}`, updateData)
          .then((bs: any) => {
            console.log('fire_other_15 UPDATED: ' + JSON.stringify(bs));
            this.presentToast('Other successfully saved!');
            this.modalController.dismiss();
          }, err => {
            console.log('fire_other_15 ERROR: ' + JSON.stringify(err));
            this.presentToast('Other could not be saved!');
          });
          ...
          const isSync = 'No';
          // eslint-disable-next-line max-len
          this.database.executeSql(`INSERT INTO fire_other_15 (service_type_id, site_id, service_cert_id, tech_id, location_1, functional_1, comment_1, location_2, functional_2, comment_2, location_3, functional_3, comment_3, location_4, functional_4, comment_4, location_5, functional_5, comment_5, isSync, created_on) VALUES ('${this.stairPressure.service_type_id}','${this.stairPressure.site_id}','${this.stairPressure.service_cert_id}','${this.stairPressure.tech_id}','${this.location1}','${this.functional1}','${this.comment1}','${this.location2}', '${this.functional2}', '${this.comment2}', '${this.location3}', '${this.functional3}', '${this.comment3}', '${this.location4}', '${this.functional4}', '${this.comment4}', '${this.location5}', '${this.functional5}', '${this.comment5}', '${isSync}','${this.stairPressure.created_on}')`, [])
          .then((bs: any) => {
            console.log('fire_other_15 ADDED: ' + JSON.stringify(bs));
            this.presentToast('Other successfully saved!');
            this.modalController.dismiss();
          }, err => {
            console.log('fire_other_15 ERROR: ' + JSON.stringify(err));
  ...
  dropTable() {
    this.database.executeSql(`DROP table fire_other_15 `, [])
      .then((resAdd: any) => {
        console.log('TABLE fire_other_15 REMOVED: ' + JSON.stringify(resAdd));
      });
  }
