...
@Component({
  selector: 'app-liner-heat',
  templateUrl: './liner-heat.page.html',
  styleUrls: ['./liner-heat.page.scss'],
})
export class LinerHeatPage implements OnInit {
  beam: any = {};
  certID: any;
  cert: any;
  networkStatus: any;
  ...
  submitBeams() {
    ...
    if (this.networkStatus === 'none') {
      const bsSql = 'SELECT * FROM fire_liner_heat WHERE service_cert_id=?';
      this.database.executeSql(bsSql, [this.certID]).then((bsRes: any) => {
        ...
        if (bsRes.rows.length > 0) {
          ...
          .then((bs: any) => {
            ...
            console.log('fire_liner_heat ERROR: ' + JSON.stringify(err));
  ...
  dropTable() {
    this.database.executeSql(`DROP table fire_liner_heat `, [])
      .then((resAdd: any) => {
        console.log('TABLE fire_liner_heat REMOVED: ' + JSON.stringify(resAdd));
      });
  }
