...
@Component({
  selector: 'app-auto-manual-switch',
  templateUrl: './auto-manual-switch.page.html',
  styleUrls: ['./auto-manual-switch.page.scss'],
})
export class AutoManualSwitchPage implements OnInit {
  stairPressure: any = {};
  certID: any;
  cert: any;
  networkStatus: any;
  ...
  constructor(
    ...
    this.sqlite.create({
      ...
      this.database = db;
      //  this.dropTable();
      this.database.executeSql(`CREATE TABLE IF NOT EXISTS fire_auto_manual_switch  (
        ...
        created_on TEXT)`, []).then((res: any) => {
          console.log('fire_auto_manual_switch table Created: ' + JSON.stringify(res));
        });
        ...
        if (this.networkStatus === 'none') { //Offline
          const stairPressureSql = 'SELECT * FROM fire_auto_manual_switch WHERE service_cert_id=?';
  ...
  submitstairPressures() {
    ...
    if (this.networkStatus === 'none') {
      const bsSql = 'SELECT * FROM fire_auto_manual_switch WHERE service_cert_id=?';
      this.database.executeSql(bsSql, [this.certID]).then((bsRes: any) => {
        ...
        if (bsRes.rows.length > 0) {
          ...
          // eslint-disable-next-line max-len
          this.database.executeSql(`UPDATE fire_auto_manual_switch SET service_type_id=?, site_id=?, service_cert_id=?, tech_id=?, location_1=?, functional_1=?, comment_1=?, location_2=?, functional_2=?, comment_2=?, location_3=?, functional_3=?, comment_3=?, location_4=?, functional_4=?, comment_4=?, location_5=?, functional_5=?, comment_5=? WHERE id=${pressure.id}`, updateData)
          .then((bs: any) => {
            console.log('fire_auto_manual_switch UPDATED: ' + JSON.stringify(bs));
            this.presentToast('Auto manual switch successfully saved!');
            this.modalController.dismiss();
          }, err => {
            console.log('fire_auto_manual_switch ERROR: ' + JSON.stringify(err));
            this.presentToast('Auto manual switch could not be saved!');
          });
  ...
  dropTable() {
    this.database.executeSql(`DROP table fire_auto_manual_switch `, [])
      .then((resAdd: any) => {
        console.log('TABLE fire_auto_manual_switch REMOVED: ' + JSON.stringify(resAdd));
      });
  }
}
