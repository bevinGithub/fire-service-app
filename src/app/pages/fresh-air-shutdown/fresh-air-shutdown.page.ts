...
@Component({
  selector: 'app-fresh-air-shutdown',
  templateUrl: './fresh-air-shutdown.page.html',
  styleUrls: ['./fresh-air-shutdown.page.scss'],
})
export class FreshAirShutdownPage implements OnInit {
  stairPressure: any = {};
  certID: any;
  cert: any;
  networkStatus: any;
  ...
  constructor(
    ...
    this.sqlite.create({
      ...
      this.database = db;
      //  this.dropTable();
      this.database.executeSql(`CREATE TABLE IF NOT EXISTS fire_fresh_air_shutdown  (
        ...
        created_on TEXT)`, []).then((res: any) => {
          console.log('fire_fresh_air_shutdown table Created: ' + JSON.stringify(res));
        });
    });
  }
  ...
  ionViewWillEnter(){
    ...
    if (this.networkStatus === 'none') { //Offline
      const stairPressureSql = 'SELECT * FROM fire_fresh_air_shutdown WHERE service_cert_id=?';
  ...
  submitstairPressures() {
    ...
    if (this.networkStatus === 'none') {
      const bsSql = 'SELECT * FROM fire_fresh_air_shutdown WHERE service_cert_id=?';
      this.database.executeSql(bsSql, [this.certID]).then((bsRes: any) => {
        ...
        if (bsRes.rows.length > 0) {
          ...
          // eslint-disable-next-line max-len
          this.database.executeSql(`UPDATE fire_fresh_air_shutdown SET service_type_id=?, site_id=?, service_cert_id=?, tech_id=?, location_1=?, functional_1=?, comment_1=?, location_2=?, functional_2=?, comment_2=?, location_3=?, functional_3=?, comment_3=?, location_4=?, functional_4=?, comment_4=?, location_5=?, functional_5=?, comment_5=? WHERE id=${pressure.id}`, updateData)
          .then((bs: any) => {
            console.log('fire_fresh_air_shutdown UPDATED: ' + JSON.stringify(bs));
            this.presentToast('Fresh air shutdown successfully saved!');
            this.modalController.dismiss();
          }, err => {
            console.log('fire_fresh_air_shutdown ERROR: ' + JSON.stringify(err));
            this.presentToast('Fresh air shutdown could not be saved!');
          });
          ...
          const isSync = 'No';
          // eslint-disable-next-line max-len
          this.database.executeSql(`INSERT INTO fire_fresh_air_shutdown (service_type_id, site_id, service_cert_id, tech_id, location_1, functional_1, comment_1, location_2, functional_2, comment_2, location_3, functional_3, comment_3, location_4, functional_4, comment_4, location_5, functional_5, comment_5, isSync, created_on) VALUES ('${this.stairPressure.service_type_id}','${this.stairPressure.site_id}','${this.stairPressure.service_cert_id}','${this.stairPressure.tech_id}','${this.location1}','${this.functional1}','${this.comment1}','${this.location2}', '${this.functional2}', '${this.comment2}', '${this.location3}', '${this.functional3}', '${this.comment3}', '${this.location4}', '${this.functional4}', '${this.comment4}', '${this.location5}', '${this.functional5}', '${this.comment5}', '${isSync}','${this.stairPressure.created_on}')`, [])
          .then((bs: any) => {
            console.log('fire_fresh_air_shutdown ADDED: ' + JSON.stringify(bs));
            this.presentToast('Fresh air shutdown successfully saved!');
            this.modalController.dismiss();
          }, err => {
            console.log('fire_fresh_air_shutdown ERROR: ' + JSON.stringify(err));
  ...
  dropTable() {
    this.database.executeSql(`DROP table fire_fresh_air_shutdown `, [])
      .then((resAdd: any) => {
        console.log('TABLE fire_fresh_air_shutdown REMOVED: ' + JSON.stringify(resAdd));
      });
  }
}
