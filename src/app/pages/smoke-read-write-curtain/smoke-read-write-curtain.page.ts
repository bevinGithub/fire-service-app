...
import { TagsFanDataPage } from 'src/app/modals/tags-fan-data/tags-fan-data.page';
import { TagsCurtainDataPage } from 'src/app/modals/tags-curtain-data/tags-curtain-data.page';

@Component({
  selector: 'app-smoke-read-write-curtain',
  templateUrl: './smoke-read-write-curtain.page.html',
  styleUrls: ['./smoke-read-write-curtain.page.scss'],
})
export class SmokeReadWriteCurtainPage implements OnInit {
  url = environment.url;
  techID: any;
  deviceData: any;
  config: any;
  ...
  public siteID: any;
  public certID: any;
  public curtainNumber: any;
  public location: any;
  public tagDate: any;
  public serviceID: any;
  public serviceTypeID: any;
  public curtainCode: any;
  public cpID: any;
  ...
  ionViewWillEnter(){
    ...
    if (this.state === 'read') {
      this.pageTitle = 'READING RFID: CURTAINS';
      this.readingTag = true;
      this.nfcReadData();
    }
    if (this.state === 'write') {
      this.pageTitle = 'WRITING TO RFID: CURTAINS';
  ...
  async getTagData(tag) {
    ...
    if (tag.ndefMessage[2]) {
      this.curtainCode  = this.nfc.bytesToString(tag.ndefMessage[2].payload).substring(3);
    } else {
      this.curtainCode = '';
    }
    if (tag.ndefMessage[1]) {
      this.curtainNumber  = this.nfc.bytesToString(tag.ndefMessage[1].payload).substring(3);
    } else {
      this.curtainNumber = '';
    }
    ...
    console.log('SiteID: ' + this.siteID + ' ---Fan Code: ' + this.curtainCode + ' ---Fan Number: ' + this.curtainNumber  + '----Location: ' + this.location + '----: Date Scanned: '
    ...
    if (this.siteID !== '' && this.serviceID === '' && this.serviceTypeID === '') {
      ...
      const readData = {
        ...
        curtainCode: this.curtainCode,
        curtainNumber: this.curtainNumber,
        location: this.location,
        techID: this.techID,
        cpID: this.cpID,
      };
      console.log(readData);
      this.http.post(this.url + 'sp-post-tag-curtain-data.php', readData).subscribe((data: any) => {
        console.log(data);
...
writeTag() {
  ...
  this.platform.ready().then(() => {
    ...
    this.listener = this.nfc.addNdefListener((res: any) => {
      ...
      const curtainNumber = this.ndef.textRecord(this.tag.curtainNumber);
      const curtainCode = this.ndef.textRecord(this.tag.curtainCode);
      ...
      this.nfc.write([siteID,curtainNumber, curtainCode,location,date,serviceID,serviceTypeID,cpID]).then(success => {
