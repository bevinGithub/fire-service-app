...
@Component({
  selector: 'app-air-sampling',
  templateUrl: './air-sampling.page.html',
  styleUrls: ['./air-sampling.page.scss'],
})
export class AirSamplingPage implements OnInit {
  beam: any = {};
  certID: any;
  cert: any;
  networkStatus: any;
  ...
  constructor(
    ...
    this.sqlite.create({
      ...
      this.database = db;
      //  this.dropTable();
      this.database.executeSql(`CREATE TABLE IF NOT EXISTS fire_air_sampling_template  (
        ...
        detection_created TEXT)`, []).then((res: any) => {
          console.log('Air Sampling table Created: ' + JSON.stringify(res));
        });
    });
   }
  ...
  ionViewWillEnter(){
    ...
    if (this.networkStatus === 'none') { //Offline
      const beamSql = 'SELECT * FROM fire_air_sampling_template WHERE service_cert_id=?';
  ...
  submitBeams() {
    ...
    if (this.networkStatus === 'none') {
      const bsSql = 'SELECT * FROM fire_air_sampling_template WHERE service_cert_id=?';
      this.database.executeSql(bsSql, [this.certID]).then((bsRes: any) => {
        ...
        if (bsRes.rows.length > 0) {
          ...
          // eslint-disable-next-line max-len
          this.database.executeSql(`UPDATE fire_air_sampling_template SET service_type_id=?, site_id=?, service_cert_id=?, tech_id=?, type_make=?, units=?, units_tested=?, comment=? WHERE id=${beam.id}`, updateBeam)
          .then((bs: any) => {
            console.log('AIR SAMLING UPDATED: ' + JSON.stringify(bs));
            this.presentToast('Air sampling successfully saved!');
            this.modalController.dismiss();
          }, err => {
            console.log('AIR UPDATE ERROR: ' + JSON.stringify(err));
            this.presentToast('Air sampling could not be saved!');
          });
        } else {
          if (this.beam.type_make) {
            this.typeMake = this.beam.type_make;
  ...
  dropTable() {
    this.database.executeSql(`DROP table fire_air_sampling_template `, [])
      .then((resAdd: any) => {
        console.log('TABLE air_sampling REMOVED: ' + JSON.stringify(resAdd));
      });
  }
