...
@Component({
  selector: 'app-offline-service-cards',
  templateUrl: './offline-service-cards.page.html',
  styleUrls: ['./offline-service-cards.page.scss'],
})
export class OfflineServiceCardsPage implements OnInit {
  services: any;
  services2: any;
  techID: any;
  url = environment.url;
  ...
  async syncServiceCard(cert) {
    const alert = await this.alertController.create({
      header: 'Sync Confirmation!',
      message: 'Are you sure you want to sync this record?',
      buttons: [
        {
          text: 'Cancel',
          role: 'cancel',
  ...
  async processSyncddd(data) {
    ...
    // console.log(this.url + 'sync-service-card.php');
    // this.http.post(this.url + 'sync-service-card.php', cert).subscribe((card: any) => {
    //   console.log('Synced.....' + JSON.stringify(card));
    // });
    loading.dismiss();
  }
