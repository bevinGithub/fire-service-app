...
export class CleaningDevicesPage implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.networkStatus === 'none') { //Offline
      ...
      this.sqlite.create({
        ...
        this.database.executeSql(`SELECT * FROM fire_template_device_loops_table WHERE service_type_id=? AND site_id=? AND panel_id=?`, [this.serviceTypeID, this.siteID,this.panelID])
        .then((res: any) => {
  ...
  onSubmit() {
    ...
    for(let i=0; i < this.fieldsCounter.length; i++) {
      ...
      // eslint-disable-next-line max-len
      this.database.executeSql(`UPDATE fire_template_device_loops_table SET na=?, cleaned=?, tested=?, correct_message=?, new_message=?, isSync=? WHERE id=${id}`, updateValues)
      .then((res: any) => {
        console.log(res);
      }, err => {
