...
export class PreServiceCheckPage implements OnInit {
  ...
  constructor(
    ...
    this.sqlite.create({
      ...
      this.database = db;
      //  this.dropTable();
      this.database.executeSql(`CREATE TABLE IF NOT EXISTS fire_sp_template_panels_post_data (
        ...
        panel_date TEXT)`, []).then((res: any) => {
          console.log('fire_sp_template_panels_post_data table Created: ' + JSON.stringify(res));
        });
        ...
        if (this.networkStatus === 'none') { //Offline
          const serviceCheckSql = 'SELECT * FROM fire_sp_template_panels_post_data WHERE service_cert_id=?';
          ...
          const sqlPQeury = 'SELECT * FROM fire_sp_template_panels WHERE id=?';
  ...
  submitServiceCheck() {
    ...
    if (this.networkStatus === 'none') {
      ...
      const bsSql = 'SELECT * FROM fire_sp_template_panels_post_data WHERE service_cert_id=?';
